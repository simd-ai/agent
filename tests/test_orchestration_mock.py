...
"""Tests for orchestration with mock providers and sandbox."""
...
class TestOrchestrationCodegenRun:
    ...
    async def test_codegen_with_mock_sandbox_success(
        self,
        codegen_request,
        mock_websocket,
        run_id,
        ...
        """Test codegen run with successful mock sandbox."""
        store = MagicMock(spec=EventStore)
        store.create_run = AsyncMock()
        store.append_event = AsyncMock()
        store.finalize_run = AsyncMock()
        ...
        # Inject mock sandbox
        orchestrator._sandbox = mock_sandbox_client
    ...
    async def test_codegen_retries_on_failure(
        self,
        codegen_request,
        mock_websocket,
        run_id,
        mock_failing_sandbox_client,
    ):
        """Test that codegen retries on sandbox failure."""
        store = MagicMock(spec=EventStore)
        store.create_run = AsyncMock()
        store.append_event = AsyncMock()
        store.finalize_run = AsyncMock()
        ...
        # Inject failing sandbox
        orchestrator._sandbox = mock_failing_sandbox_client
        
        result = await orchestrator.run()
        
        # Should fail after max retries
        assert result.status == RunStatus.FAILED
        assert result.retries == codegen_request.constraints.max_retries
    
    async def test_unclear_simulation_type(
        self,
        mock_websocket,
        run_id,
    ):
        """Test handling of unclear simulation type."""
        request = StartRequest(
            op=Operation.CFD_CODEGEN_RUN,
            provider="mock",
            user_requirements="Do something with fluids",  # Very vague
            simulation_config={},  # No geometry hints
        )
        ...
        # Should return not_clear status
...
class TestErrorHandling:
    ...
    async def test_exception_emits_failure_event(
        ...
        # Make sandbox raise an exception
        orchestrator._sandbox = MagicMock()
        orchestrator._sandbox.submit_run = AsyncMock(side_effect=Exception("Connection failed"))
        orchestrator._sandbox.close = AsyncMock()
