...
class TestStartRequest:
    ...
    def test_minimal_request(self):
        ...
        assert request.provider == "gemini"  # default
...
class TestAgentEvent:
    ...
    def test_event_to_ws_message(self):
        ...
        event = AgentEvent(
            run_id=run_id,
            seq=1,
            type=EventTypes.SIM_SUBMITTED,
            message="Submitted to simulation server",
            payload={"sim_run_id": "sim-123"},
        )
        ...
        assert msg["seq"] == 1
        assert msg["type"] == EventTypes.SIM_SUBMITTED
        assert msg["message"] == "Submitted to simulation server"
        assert msg["payload"]["sim_run_id"] == "sim-123"
        assert "ts" in msg
