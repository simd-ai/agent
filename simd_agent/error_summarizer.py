...
class ErrorSummarizer:
    ...
    async def summarize(
        ...
        Args:
            logs: Execution logs from sandbox
            exit_code: Process exit code
            current_files: Optional dictionary of current case files
            
        Returns:
            ErrorSummary with root cause and actionable changes
        """
        # First try pattern matching
        summary = self._pattern_match(logs, exit_code)
        
        # If pattern matching didn't find anything specific and LLM is enabled
        if summary.confidence < 0.5 and self.use_llm and self.code_generator:
            summary = await self._llm_analyze(logs, exit_code, current_files)
