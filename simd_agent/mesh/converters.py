...
def _poly_from_faces(
    points: np.ndarray,
    face_cells: np.ndarray,
    verts_per_face: int,
) -> pv.PolyData:
    """Create PolyData from points and face connectivity (triangles or quads).

    Args:
        points: (N, 3) vertex coordinates
        face_cells: (M, verts_per_face) face vertex indices
        verts_per_face: 3 for triangles, 4 for quads
    ...
    if face_cells.size == 0:
        return pv.PolyData()
    prefix = np.full((face_cells.shape[0], 1), verts_per_face, dtype=np.int64)
    faces = np.hstack([prefix, face_cells]).ravel()
    return pv.PolyData(points, faces)


def _merge_polys(polys: List[pv.PolyData]) -> pv.PolyData:
    """Merge multiple PolyData objects into one."""
    valid = [p for p in polys if p is not None and p.n_points > 0]
    if not valid:
        return pv.PolyData()
    if len(valid) == 1:
        return valid[0]
...
# Surface cell types recognised from Gmsh meshes
_SURFACE_CELL_TYPES = {
    "triangle": 3,
    "triangle6": 3,
    "quad": 4,
...
def convert_gmsh_msh(msh_path: Path) -> Tuple[pv.PolyData, Dict[str, pv.PolyData]]:
    ...
    Gmsh .msh files contain explicit boundary faces (triangles or quads) with
    physical groups.  We build patch PolyData from those surface cells.
    ...
    # Collect indices of all surface cell blocks (triangles AND quads)
    surface_block_indices: List[int] = []
    for i, cb in enumerate(mesh.cells):
        if cb.type in _SURFACE_CELL_TYPES:
            surface_block_indices.append(i)

    patches: Dict[str, pv.PolyData] = {}

    if len(surface_block_indices) > 0:
        ...
        all_polys: List[pv.PolyData] = []
        # Accumulate per-patch faces across all surface blocks
        patch_polys: Dict[str, List[pv.PolyData]] = {}

        for idx in surface_block_indices:
            cb = mesh.cells[idx]
            cells = np.asarray(cb.data, dtype=np.int64)
            # For higher-order elements, use only corner nodes
            vpf = _SURFACE_CELL_TYPES[cb.type]
            cells_corners = cells[:, :vpf]

            poly = _poly_from_faces(pts, cells_corners, vpf)
            ...
            if gmsh_phys_lists is not None and idx < len(gmsh_phys_lists):
                ...
                for tag in np.unique(tags):
                    ...
                    name = safe_name(name)
                    sub_cells = cells_corners[tags == tag]
                    if sub_cells.size == 0:
                        continue
                    sub_poly = _poly_from_faces(pts, sub_cells, vpf)
                    patch_polys.setdefault(name, []).append(sub_poly)

        merged = _merge_polys(all_polys)

        # Merge per-patch poly lists (a patch may span both tri and quad blocks)
        for name, polys in patch_polys.items():
            patches[name] = _merge_polys(polys)
