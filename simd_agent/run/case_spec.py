...
class CaseSpec:
    ...
    # These are set by build_case_spec when real checkMesh data is available.
...
def _mesh_quality_decisions(
    check_mesh: Any,
) -> dict[str, Any]:
    """Derive numerical strategy from real OpenFOAM checkMesh metrics.

    Returns a dict with keys: use_simplec, n_non_ortho_correctors,
    mesh_quality_tier, mesh_max_non_orthogonality, mesh_max_skewness,
    mesh_max_aspect_ratio.

    When no real checkMesh data is available (metrics are None/0), returns
    conservative defaults: standard SIMPLE (no SIMPLEC), 1 non-ortho
    corrector, tier "unknown".
    """
    defaults = {
        "use_simplec": False,
    ...
    logger.info(f"[MESH_QUALITY] Raw metrics: non_ortho={non_ortho}, skew={skew}, aspect={aspect}")
...
def build_case_spec(solver: str, validated_config: dict[str, Any]) -> CaseSpec:
    ...
    logger.info(f"[CASE_SPEC] top-level max_iterations={validated_config.get('max_iterations')}, end_time={validated_config.get('end_time')}")

    if not transient:
        # Steady solvers: endTime = iteration count.
        # Priority: explicit max_iterations > endTime (which IS iterations for steady)
        end_time = float(
        ...
        # Priority: explicit end_time > max_iterations (as fallback)
        # Skip endTime if it looks like a placeholder (exactly 10 with no
        # user-provided end_time) — prefer max_iterations in that case.
        _raw_end_time = (
        ...
        if _raw_end_time:
            ...
            # If both endTime and max_iterations are sent, and endTime is the
            # default placeholder (10), prefer max_iterations converted to
            # physical time via delta_t.
    ...
    # ── Respect plugin's required_files() — if the plugin excludes a system
    # file (e.g. simpleFoam generates fvSolution deterministically and removes
    # it from required_files), CaseSpec must not include it either, otherwise
    # the parallel codegen will waste an LLM call generating a file that the
    # validator will throw away.
    try:
    ...
    # Extract real OpenFOAM checkMesh metrics (if available) and derive
    # numerical strategy: SIMPLEC decision, non-ortho correctors, tier.
