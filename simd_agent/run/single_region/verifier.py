# simd_agent/run/single_region/verifier.py
"""Single-region rule-based verifier.

Lifted verbatim from the legacy :mod:`simd_agent.run.code_verifier`
rule-based path so the single-region case tree (flat ``0/<field>``,
``system/<dict>``, ``constant/<dict>``) has a dedicated module.  The
multi-region equivalent is :mod:`simd_agent.run.multi_region.verifier`;
:class:`simd_agent.run.code_verifier.CodeVerifier` is the dispatcher
that picks one or the other based on
``plugin.is_multi_region``.

These checks are fast and deterministic — no LLM.  They run after
:meth:`SolverPlugin.validate_full` (which auto-fixes most known
mistakes) and surface anything that survives so the self-healing loop
can regenerate with explicit guidance.
"""
...
from simd_agent.run.solver_selector import (
    ALLOWED_SOLVERS,
    ENERGY_SOLVERS,
    GRAVITY_SOLVERS,
    P_RGH_SOLVERS,
...
HEAT_TRANSFER_SOLVERS = ENERGY_SOLVERS
...
TRANSIENT_SOLVERS = ALLOWED_SOLVERS - STEADY_SOLVERS
...
async def verify(
    ...
    """Run single-region rule-based checks and return a consolidated result.

    Signature matches the multi-region :func:`simd_agent.run.multi_region.verifier.verify`
    so the :class:`simd_agent.run.code_verifier.CodeVerifier` dispatcher
    can swap implementations transparently.
    """
    issues = _rule_based_checks(files, validated_config, solver)
    ...
    for issue in issues:
        icon = (
            ...
            else "⚠️ " if issue.severity == "warning"
            else "ℹ️ "
        )
    ...
    return VerificationResult(passed=passed, issues=issues, summary=summary)
...
def _rule_based_checks(
    ...
    if heat_transfer and solver not in HEAT_TRANSFER_SOLVERS:
    ...
    if time_stepping == "transient" and solver in STEADY_SOLVERS:
    ...
    # Boussinesq solvers solve energy but use constant/transportProperties
    # (ν, β, T_ref, Pr, Prt) — they do NOT need thermophysicalProperties,
    # because density is treated as constant ρ₀ in the Boussinesq
    # approximation.  Skip the thermo check for them.
    _is_boussinesq = "Boussinesq" in solver
    if (
        solver in THERMO_SOLVERS
        and not _is_boussinesq
        and "constant/thermophysicalProperties" not in files
    ):
        issues.append(
            VerificationIssue(
                severity="critical",
    ...
    if _is_boussinesq and "constant/transportProperties" not in files:
        issues.append(
            VerificationIssue(
                severity="critical",
                category="missing_constant_dicts",
                message=f"'{solver}' requires 'constant/transportProperties' (ν, β, T_ref, Pr, Prt) but it is missing.",
                fix_suggestion="Generate 'constant/transportProperties' with transportModel Newtonian + beta + TRef.",
            )
        )
    ...
    for path, content in files.items():
        ...
        # ── 9. Algorithm block ────────────────────────────────────────
        # *SimpleFoam → SIMPLE block, *PimpleFoam → PIMPLE block,
        # chtMultiRegionFoam (transient) → PIMPLE.  Suffix is the
        # authoritative signal — works for all current variants
        # (rho*, buoyant*, buoyantBoussinesq*, chtMultiRegion*).
        ...
        if fv_sol:
            ...
            if needs_simple and "SIMPLE" not in fv_sol:
                issues.append(VerificationIssue(
            ...
            elif needs_pimple and "PIMPLE" not in fv_sol:
                issues.append(VerificationIssue(
                    severity="warning", category="missing_algorithm_block",
