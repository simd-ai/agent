...
def _file_doc_relpath(file_path: str) -> str:
    """Map an OpenFOAM case file path to its per-file solver doc relative path.

    system/fvSchemes                         → system/fvSchemes.md
    constant/g                               → constant/g.md
    constant/thermophysicalProperties.water  → constant/thermophysicalProperties.md
    0/U                                      → fields/U.md
    0/alpha.water                            → fields/alpha.md
    """
    ...
    if file_path.startswith("constant/"):
        ...
        # Per-phase thermo files (e.g. thermophysicalProperties.water) → same doc as base
    ...
    if file_path.startswith("0/"):
        rel = file_path.split('/', 1)[1]
        # alpha.water, alpha.liquidNitrogen, alpha.liquidNitrogen.orig, etc. → fields/alpha.md
...
def _load_solver_base(solver: str) -> str:
    """Load the short solver-identity doc (_solver.md), falling back to legacy monolithic file."""
    base = SOLVERS_DIR / solver / "_solver.md"
    if base.exists():
        return base.read_text(encoding="utf-8")
    legacy = SOLVERS_DIR / f"{solver}.md"
    return legacy.read_text(encoding="utf-8") if legacy.exists() else ""


def _load_solver_file_doc(solver: str, file_doc_relpath: str) -> str:
    """Load a per-file solver doc. Returns empty string when the file doesn't exist."""
    p = SOLVERS_DIR / solver / file_doc_relpath
    if p.exists():
...
def validate_generated_files(
    ...
    # matrix has a zero or negative diagonal (common when flow diverges).
    # Fix: override coarsestLevelCorr to use smoothSolver+symGaussSeidel, which is
    # robust against ill-conditioned matrices.
    fv_solution = fixed_files.get("system/fvSolution", "")
    if fv_solution and solver in _COMPRESSIBLE_ENERGY_SOLVERS:
        if "GAMG" in fv_solution and "coarsestLevelCorr" not in fv_solution:
            ...
            # Inject inside the first GAMG block for p (right before the closing brace)
            fv_solution = re.sub(
                r'(solver\s+GAMG\s*;)',
                r'\1' + _corr_block,
                fv_solution,
            ...
            issues.append(ValidationIssue(
                "warning", "system/fvSolution",
                "Injected coarsestLevelCorr into GAMG p solver to prevent DICPreconditioner SIGFPE "
                "when the coarsest agglomeration matrix is ill-conditioned.",
                fix="Added coarsestLevelCorr { solver smoothSolver; smoother symGaussSeidel; }"
            ))
...
def _eos_for_liquid(rho: float, inlet_temp: float | None, has_energy: bool) -> str:
    ...
    """
    if rho <= 200.0:
        # Low density → likely a gas or supercritical fluid; caller should use perfectGas
        return "rhoConst"
...
_FILE_HINTS: dict[str, str] = {
    ...
    "0/U": (
        ...
        "  - massFlowRate <kg/s>; rho rho; rhoInlet <density>; value uniform (0 0 0);\n"
    ...
    "constant/thermophysicalProperties": (
        ...
        "         where a1 = dρ/dT (LN2/LOX: -4.7; LH2: -0.7; water/oil: -0.5)\n"
        "         and a0 = rho_inlet - a1 * T_inlet\n"
        "  C. rho > 200 AND isothermal: heRhoThermo + rhoConst + const transport\n\n"
...
class GenAICodeGenerator:
    ...
    def _build_shared_context(
        ...
        """
        solver_instructions = _load_solver_base(solver)
    ...
    def _build_single_file_prompt(
        ...
        if case_spec:
            ...
            _file_doc_rel = _file_doc_relpath(file_path)
            _file_doc = _load_solver_file_doc(solver, _file_doc_rel)
            if _file_doc:
                context_section += (
    ...
    def _brief_solver_note(self, file_path: str, cs: CaseSpec) -> str:
        ...
        if file_path == "system/fvOptions":
            ...
            # Fallback max: 80% of EOS ceiling (safe for icoPolynomial) or unconstrained
            t_max_fallback = ceiling * 0.8 if ceiling else 100000.0
            ceiling_note = (
