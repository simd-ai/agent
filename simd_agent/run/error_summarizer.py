...
"""Error summarizer agent for analyzing simulation execution failures."""
...
class ErrorSummarizer:
    """Analyzes simulation execution failures and provides actionable fixes.
    ...
    async def summarize(
        ...
        """Analyze simulation execution logs and produce error summary.
    ...
    async def _llm_diagnose(
        ...
        """Use the LLM provider to diagnose the OpenFOAM error with full case context."""
        try:
            from simd_agent.llm import get_provider
            provider = get_provider()
            client = provider.client
            genai_types = provider.types
            model_name = provider.models["default"]
            ...
            prompt = (
                ...
                "  • The OpenFOAM solver (application name) has already been "
                "    chosen by the orchestrator and is LOCKED for this run. "
                "    NEVER suggest changing the solver. NEVER propose another "
                "    solver name in AFFECTED_FILES, FIX_1, or FIX_2.\n"
                "  • If the failure looks like it's caused by the wrong solver "
                "    being chosen, treat that as out-of-scope and report it in "
                "    ROOT_CAUSE only — do NOT propose a swap.\n"
                "  • Fixes must be confined to the case files listed below "
                "    (BC types, discretisation schemes, relaxation factors, "
                "    fvOptions limits, initial fields, mesh quality, etc.).\n\n"
                ...
                "NO `application` field of controlDict, NO solver-name swaps>\n"
                "FIX_1: <specific change to make to a SETTING or BC, never the solver>\n"
                "FIX_2: <optional second change, same rule>\n"
                "CONFIDENCE: <0.0-1.0>"
            )
