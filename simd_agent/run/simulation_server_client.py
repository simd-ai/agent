...
- GET  /api/run/{run_id}/vtk-results                 - Trigger foamToVTK + return field metadata + surface VTP URL
- GET  /api/run/{run_id}/vtk/surface.vtp             - Serve merged surface VTP (latest timestep)
- GET  /api/run/{run_id}/vtk-timesteps/index.json    - Precomputed index: all timestep VTPs + field metadata
- GET  /api/run/{run_id}/vtk-timesteps/{filename}    - Serve a specific precomputed VTP by stable filename
- GET  /api/run/{run_id}/vtk-timestep/{t}/surface.vtp - On-demand per-timestep VTP (cached after first call)
...
class SimulationServerClient:
    ...
    HTTP client for running OpenFOAM simulations.
    ...
    async def check_mesh(
        ...
        """Send mesh to sim server for OpenFOAM checkMesh quality analysis.

        The sim server runs gmshToFoam (for .msh) then ``checkMesh -allGeometry``
        and returns parsed quality metrics as structured JSON.

        Args:
            mesh_bytes: Raw mesh file content
            filename: Original filename (extension determines conversion method)

        Returns:
            Dict with max_non_orthogonality, avg_non_orthogonality,
        ...
        try:
            response = await client.post(
                "/api/mesh/check",
                files={"file": (filename, mesh_bytes)},
                timeout=httpx.Timeout(120.0),
            )
            response.raise_for_status()
            result = response.json()
            logger.info(
                f"[SIM_SERVER] checkMesh result: "
                f"non_ortho={result.get('max_non_orthogonality')}, "
                f"skewness={result.get('max_skewness')}, "
                f"aspect_ratio={result.get('max_aspect_ratio')}, "
                f"mesh_ok={result.get('mesh_ok')}"
            )
            return result
        except httpx.HTTPStatusError as e:
            # Preserve the response body for better diagnostics
            detail = ""
            try:
                ...
                pass
            logger.error(
                f"[SIM_SERVER] checkMesh failed: HTTP {e.response.status_code} — {detail}"
            )
            raise SimulationServerError(
                f"checkMesh failed (HTTP {e.response.status_code}): {detail}"
            )
        except httpx.ConnectError as e:
            logger.error(f"[SIM_SERVER] checkMesh connection error: {e}")
            raise SimulationServerError(
                f"Cannot reach simulation server at {self.base_url}: {e}"
            )
        except httpx.TimeoutException as e:
            logger.error(f"[SIM_SERVER] checkMesh timeout: {e}")
            raise SimulationServerError(
                f"checkMesh timed out after 120s — mesh may be too large"
            )
        except Exception as e:
            logger.error(f"[SIM_SERVER] checkMesh error: {e}")
            raise SimulationServerError(f"checkMesh failed: {e}")
    ...
    async def get_vtk_results(self, run_id: str) -> dict:
        ...
        Calls GET /api/run/{run_id}/vtk-results on the simulation server, which
        reads the OpenFOAM case and merges all boundary patches into one
        surface.vtp.
    ...
    async def cancel_run(self, run_id: str) -> bool:
        """Cancel a running simulation on the sim server.

        Sends POST /api/run/{run_id}/cancel which kills the solver process.

        Args:
            run_id: The simulation run ID

        Returns:
            True if the solver process was killed
        """
        client = await self._get_client()
        try:
            response = await client.post(f"/api/run/{run_id}/cancel", timeout=10.0)
            response.raise_for_status()
            result = response.json()
            cancelled = result.get("cancelled", False)
            logger.info(f"[SIM_SERVER] Cancel run {run_id}: cancelled={cancelled}")
            return cancelled
        except httpx.HTTPStatusError as e:
            logger.warning(f"[SIM_SERVER] Cancel failed: {e.response.status_code}")
            return False
        except Exception as e:
