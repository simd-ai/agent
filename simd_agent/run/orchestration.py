...
from simd_agent.telemetry import get_telemetry, RunStarted, RunCompleted, RunFailed, SolverSelected, SimulationResubmitted
...
class Orchestrator:
    ...
    def __init__(
        ...
        Args:
            ...
            cancelled: Optional cancellation event; when set the orchestrator
                       should stop at the next checkpoint.
        """
        self.run_id = run_id
        ...
        self._solver_plugin: SolverPlugin | None = None # set from registry after solver selection
        ...
        # Non-blocking local progress writer (replaces per-row DB inserts)
        from simd_agent.progress_store import ProgressWriter
        self._progress_writer = ProgressWriter(run_id)

        # Convergence assessment state
        self._all_progress: list[dict[str, Any]] = []   # full accumulator for convergence
        self._convergence_step_counter = 0               # counts steps since last emit
        self._last_convergence: dict[str, Any] | None = None  # latest assessment
    ...
    async def run(self) -> FinalResult:
        ...
        if self.request.op != Operation.CFD_RESUBMIT:
        ...
        try:
            ...
            elif self.request.op == Operation.CFD_RESUBMIT:
                logger.info("[ORCHESTRATOR] Running CFD_RESUBMIT operation...")
                get_telemetry().capture(
                    SimulationResubmitted(solver=self._selected_solver),
                    user_id=self.request.metadata.user_id,
                )
                return await self._run_resubmit()
            else:
            ...
            get_telemetry().capture(RunFailed(
            ...
            await self._progress_writer.finalize()
            if self._sim_server:
    ...
    async def _run_codegen_and_simulate(self) -> FinalResult:
        ...
        # ── Telemetry: solver selected + run started ─────────────────────
        _vc = self._lint_result.validated_config if self._lint_result else {}
        _physics = _vc.get("physics", {})
        _fluid = _vc.get("fluid", {})
        _turb = _vc.get("turbulence", {})
        _mesh = _vc.get("mesh", {})
        ...
        get_telemetry().capture(RunStarted(
            solver=solver,
            flow_regime=_physics.get("flow_regime"),
            time_scheme=_physics.get("time_scheme"),
            compressibility=_physics.get("compressibility"),
            heat_transfer=_physics.get("heat_transfer_enabled"),
            gravity=_physics.get("gravity_enabled"),
            fluid=_fluid.get("name"),
            turbulence_model=_turb.get("model"),
            mesh_cells=_mesh.get("check_mesh", {}).get("cells") if isinstance(_mesh.get("check_mesh"), dict) else None,
            patch_count=len(_mesh.get("patches", [])) if isinstance(_mesh.get("patches"), list) else None,
        ), user_id=_uid)

        # ── Resolve solver plugin from registry ────────────────────────────
        registry = get_registry()
        self._solver_plugin = registry.get(solver)
        if self._solver_plugin:
            logger.info(
                f"[CODEGEN] Solver plugin resolved: {self._solver_plugin.name} "
                f"(compressible={self._solver_plugin.is_compressible}, "
                f"energy={self._solver_plugin.supports_energy})"
            )
        else:
            logger.info(
                f"[CODEGEN] No plugin for '{solver}' — using legacy prompt pack + validation"
            )
        ...
        while self._retries <= max_retries:
            ...
            try:
                ...
                # ── On retry: clear stale sim_progress data ────────────────
                # A new simulation attempt is about to start; the residuals from the
                # failed run must not mix with the new run's data.
                if self._iteration > 1:
                    await self._progress_writer.clear()
                    self._all_progress.clear()
                    self._convergence_step_counter = 0
                ...
                await self.event_bus.emit_thinking_started("Generating code…")
                try:
                ...
                if new_files:
                    print(f"[CODEGEN] New files from LLM: {list(new_files.keys())}")
                    self._accumulated_files.update(new_files)
                    print(f"[CODEGEN] Accumulated files after merge: {list(self._accumulated_files.keys())} ({sum(len(v) for v in self._accumulated_files.values())} chars total)")
                    # Persist to DB immediately so files survive crashes mid-retry
                    try:
                        await self.store.update_generated_files(
                            self.run_id, self._accumulated_files
                        )
                        print(f"[CODEGEN] Incremental DB save completed for run {self.run_id}")
                    except Exception as _e:
                        print(f"[CODEGEN] ERROR — incremental DB save FAILED: {_e}")
                        logger.warning("Failed to incrementally save generated_files: %s", _e)
                ...
                logger.debug(
                    "[CONTROLDICT] iter=%d max_iterations=%s end_time=%s "
                    "LLM_endTime=%s LLM_startFrom=%s LLM_startTime=%s",
                    self._iteration,
                    _vconfig.get('max_iterations', '<not set>'),
                    _vconfig.get('end_time', '<not set>'),
                    _end_time_match.group(1) if _end_time_match else 'NOT FOUND',
                    _start_from_match.group(1) if _start_from_match else 'NOT FOUND',
                    _start_time_match.group(1) if _start_time_match else 'NOT FOUND',
                )

                # ── Log generated file names and sizes ──
                logger.debug("[CODEGEN] Generated files: %s", {fp: len(fc) for fp, fc in files.items()})

                # ── Post-generation validation ──
                logger.info(f"[CODEGEN] Validating {len(files)} generated files...")

                # Single unified entry point: plugin.validate_full() composes
                ...
                # ── Sync accumulator with validator-injected files ──────
                # validate_full() may add deterministic files (e.g. fvSolution
                # for simpleFoam) that the LLM never generated.  Keep the
                # accumulator in sync so the DB record + frontend restore
                # include these files.
                _pre_acc_keys = set(self._accumulated_files.keys())
                _validator_injected: dict[str, str] = {}
                for _fp, _fc in files.items():
                    if _fp not in _pre_acc_keys or self._accumulated_files.get(_fp) != _fc:
                        _validator_injected[_fp] = _fc
                if _validator_injected:
                    self._accumulated_files.update(_validator_injected)
                    logger.info(
                        "[CODEGEN] Synced %d validator-modified files to accumulator: %s",
                        len(_validator_injected), sorted(_validator_injected.keys()),
                    )
                    # Persist updated accumulator to DB
                    try:
                        await self.store.update_generated_files(
                            self.run_id, self._accumulated_files
                        )
                    except Exception as _e:
                        logger.warning("Failed to save validator-updated files: %s", _e)

                    # Emit file_generated events so the frontend can display
                    # deterministic / validator-modified files.
                    for _fp, _fc in _validator_injected.items():
                        try:
                ...
                await self.event_bus.emit_thinking_started("Verifying code…")
                try:
                ...
                if verification:
                    ...
                    logger.debug(
                        "[VERIFY] iter=%d passed=%s summary=%s",
                        self._iteration, verification.passed, verification.summary,
                    )
                    for _vi in verification.issues:
                        _lvl = "CRITICAL" if _vi.severity == "critical" else (
                            "WARN" if _vi.severity == "warning" else "INFO"
                        )
                        logger.debug("[VERIFY] %s [%s] %s", _lvl, _vi.category, _vi.message)
                        if _vi.fix_suggestion:
                            logger.debug("[VERIFY]   fix: %s", _vi.fix_suggestion)
                ...
                logger.debug(
                    "[PACKAGE] ZIP report: mesh_id=%r raw_mesh=%r size=%d files=%d "
                    "has_msh=%s has_polyMesh=%s has_blockMesh=%s mesh_present=%s zip_files=%s",
                    mesh_id, _raw_mesh_val, len(zip_bytes), len(_zip_names),
                    _has_msh, _has_poly, _has_block, _mesh_in_zip, _zip_names,
                )
                ...
                # ── Persist case ZIP to object storage ──────────────────
                try:
                    from simd_agent.storage import get_storage
                    _storage = get_storage()
                    _case_key = f"cases/{self.run_id}/case.zip"
                    await _storage.upload(_case_key, zip_bytes, "application/zip")
                    logger.info(f"[CODEGEN] Case ZIP saved to storage: {_case_key} ({len(zip_bytes)} bytes)")
                except Exception as _store_err:
                    logger.warning(f"[CODEGEN] Failed to save case ZIP to storage: {_store_err}")
                ...
                logger.debug(
                    "[SIM_SERVER] TEST run result (iter %d): %s",
                    self._iteration, _json.dumps(sim_result, indent=2, default=str),
                )
                ...
                if sim_result["success"]:
                    ...
                    logger.debug(
                        "[SIM_SERVER] FULL run result (iter %d): %s",
                        self._iteration, _json.dumps(full_result, indent=2, default=str),
                    )
                    ...
                    if full_result["success"]:
                        ...
                        if self._all_progress and not self._last_convergence:
                            try:
                                from simd_agent.convergence import compute_convergence
                                is_transient = (
                                    self._solver_plugin.is_transient
                                    if self._solver_plugin else None
                                )
                                self._last_convergence = compute_convergence(
                                    self._all_progress, is_transient=is_transient,
                                )
                            except Exception:
                                pass
                        ...
                        await self.store.finalize_run(
                            ...
                            generated_files=self._accumulated_files,
                        )

                        # Emit final convergence before the final event
                        if self._last_convergence:
                            await self.event_bus.emit_sim_event(
                                event_type="convergence_update",
                                message=self._last_convergence["status"],
                                payload=self._last_convergence,
                                level="info",
                                persist=False,
                            )
                ...
                logger.warning(
                    ...
                    self._iteration, sim_exit_code, sim_error,
                )
                _err_detail = sim_stderr or sim_logs
                if _err_detail:
                ...
                try:
                    ...
                    if _applied_fixes:
                        ...
                        print(f"[SURGICAL] Patched files: {list(_patched_files.keys())} — accumulated now {len(self._accumulated_files)} files")
                        # Persist updated files to DB immediately
                        try:
                            await self.store.update_generated_files(
                                self.run_id, self._accumulated_files
                            )
                            print(f"[SURGICAL] Incremental DB save completed for run {self.run_id}")
                        except Exception as _e:
                            print(f"[SURGICAL] ERROR — incremental DB save FAILED: {_e}")
                            logger.warning("Failed to save generated_files after surgical fix: %s", _e)
                ...
                logger.warning(
                    "[SIM_CONN_ERROR] Simulation server connection error (iter %d): %s",
                    self._iteration, e,
                )
                if self._retries >= max_retries:
                    break
                ...
                logger.warning(
                    "[ORCHESTRATION_ERROR] Orchestration error (iter %d): %s",
                    self._iteration, e,
                )
                if self._retries >= max_retries:
                    break
        ...
        # Compute final convergence assessment even for failed runs
        if self._all_progress and not self._last_convergence:
            try:
                from simd_agent.convergence import compute_convergence
                is_transient = (
        ...
        await self.store.finalize_run(
            ...
            attempts=self._iteration,
            generated_files=self._accumulated_files or None,
        )
        ...
        get_telemetry().capture(RunFailed(
            ...
            error_summary=last_error[:200] if last_error else None,
            retry_count=self._retries,
            duration_s=(datetime.utcnow() - self.event_bus._started_at).total_seconds(),
        ), user_id=self.request.metadata.user_id)
    ...
    async def _run_resubmit(self) -> FinalResult:
        """Re-run a simulation with pre-modified files (no lint/codegen).

        Used after applying convergence recommendations (relaxation changes,
        time step adjustments, etc.) to the generated files of a parent run.
        Loads the parent run's generated files, packages with mesh, and
        submits directly to the sim server.
        """
        self._progress_writer.start()

        parent_run_id = self.request.metadata.parent_run_id
        if not parent_run_id:
            raise OrchestrationError("CFD_RESUBMIT requires metadata.parent_run_id")

        refine_strategy = self.request.metadata.refine_strategy or "none"

        # Load parent run's generated files via run_service
        from uuid import UUID as _UUID
        from simd_agent.services import run_service
        parent_run = await run_service.get(_UUID(parent_run_id))
        if not parent_run:
            raise OrchestrationError(f"Parent run {parent_run_id} not found")

        parent_files = parent_run.generated_files
        if not parent_files:
            raise OrchestrationError(f"Parent run {parent_run_id} has no generated files")

        solver = parent_run.solver or "simpleFoam"

        await self.event_bus.emit(
            "resubmit_started",
            message=f"Re-running with modified files from run {parent_run_id[:8]}"
                    + (f" (mesh refinement: {refine_strategy})" if refine_strategy != "none" else ""),
            payload={
        ...
        # Get mesh_id from simulation config, falling back to parent run's
        # lint result.  The frontend sends lintResult.validated_config as
        # simulationConfig, but it may not contain the mesh section (e.g.
        # compose-mode runs).  The parent run's lint_result always has it.
        ...
        if not mesh_id:
            # Fallback: try top-level mesh_id in simulation_config
            mesh_id = self.request.simulation_config.get("mesh_id")

        if not mesh_id and parent_run.lint_result:
            # Fallback: extract from parent run's validated_config
            parent_vc = parent_run.lint_result.get("validated_config", {})
            parent_mesh = parent_vc.get("mesh", {})
            if isinstance(parent_mesh, dict):
                mesh_id = parent_mesh.get("mesh_id") or parent_mesh.get("meshId")
            if not mesh_id:
                mesh_id = parent_vc.get("mesh_id")

        # Package with mesh
        if mesh_id:
            zip_bytes, file_list, warnings = package_simulation_case(
                generated_files=parent_files,
                mesh_id=mesh_id,
                solver=solver,
            )
        else:
        ...
        logger.info(
            f"[RESUBMIT] Packaged {len(file_list)} files, "
            f"ZIP={len(zip_bytes)} bytes, solver={solver}"
        )

        # Test run first
        await self.event_bus.emit(
            "sim_test_started",
            message="Running test validation (1 iteration)...",
            payload={"solver": solver},
        )
        test_result = await self._run_on_sim_server(
            zip_bytes, mode=SimRunMode.TEST, refine_strategy=refine_strategy,
        )

        if not test_result["success"]:
            error_msg = test_result.get("error", "Test validation failed")
            await self.event_bus.emit_run_failed(f"Test validation failed: {error_msg}")
            await self.event_bus.emit_final(
                status="failed",
                error=error_msg,
                solver=solver,
                summary="Re-run test validation failed",
            )
            return FinalResult(
                status=RunStatus.FAILED,
                error=error_msg,
                solver=solver,
                summary="Test validation failed on re-submitted case",
            )

        # Full run
        await self.event_bus.emit(
            "full_run_started",
            message="Test passed — starting full simulation",
            payload={"solver": solver},
        )

        full_result = await self._run_on_sim_server(
            zip_bytes, mode=SimRunMode.FULL, n_cores=12,
            refine_strategy=refine_strategy,
        )
        ...
        if full_result["success"]:
            ...
            # Compute convergence
            if self._all_progress:
                try:
                    from simd_agent.convergence import compute_convergence
                    self._last_convergence = compute_convergence(self._all_progress)
                except Exception:
                    pass
            ...
            return FinalResult(
                ...
                iterations=1,
                solver=solver,
                summary="Re-run completed",
            )
        else:
            error_msg = full_result.get("error", "Simulation failed")
            await self.event_bus.emit_run_failed(error_msg)
            await self.store.finalize_run(
                run_id=self.run_id,
                status=RunStatus.FAILED,
                result={"error": error_msg, "parent_run_id": parent_run_id},
                attempts=1,
                generated_files=parent_files,
            )
            await self.event_bus.emit_final(
                ...
                error=error_msg,
                solver=solver,
                summary="Re-run simulation failed",
            )
            return FinalResult(
                status=RunStatus.FAILED,
                error=error_msg,
    ...
    async def _run_on_sim_server(
        ...
        # Accumulator for fieldMinMax output parsed from run_log lines.
        # Keyed by field name → {"min": float, "max": float}.
        # Flushed into the next sim_progress_batch and then reset.
        _field_ranges: dict[str, dict[str, float]] = {}
        # Matches scalar fields: "min(p) = -0.234" or "min(alpha.water) = 0.1"
        # Skips vector fields like "min(U) = (0 0 0)" — the value must be a
        # plain number, not a parenthesised vector.
        _FIELD_MINMAX_RE = re.compile(
            r'(min|max)\(([\w.]+)\)\s*=\s*([-+]?\d+(?:\.\d+)?(?:[eE][-+]?\d+)?)'
        )

        # Accumulator for surfaceFieldValue (patch-averaged) output.
        # Keyed by patch → field → value.  e.g. {"inlet": {"p": 76.23}, "outlet": {"p": 0.15}}
        _patch_values: dict[str, dict[str, float]] = {}
        # Matches: "areaAverage(inlet) of p = 76.23"
        _SURFACE_FIELD_RE = re.compile(
            r'(?:areaAverage|areaIntegrate)\(([\w]+)\)\s+of\s+([\w.]+)\s*=\s*([-+]?\d+(?:\.\d+)?(?:[eE][-+]?\d+)?)'
        )

        # Accumulator for volFieldValue (domain-wide integrals) output.
        # Keyed by field → {"operation": str, "value": float}.
        _volume_integrals: dict[str, dict[str, Any]] = {}
        # Matches: "volAverage(p) = 101325" or "volIntegrate(alpha.water) = 0.00234"
        _VOL_FIELD_RE = re.compile(
            r'(volAverage|volIntegrate)\(([\w.]+)\)\s*=\s*([-+]?\d+(?:\.\d+)?(?:[eE][-+]?\d+)?)'
        )
        ...
        async def on_sim_event(event: SimRunEvent):
            ...
            if event.type == "run_log":
                ...
                # ── Parse fieldMinMax output lines ────────────────────────────
                # e.g. "    min(p) = -0.234 at location (…)"
                _mm = _FIELD_MINMAX_RE.search(line)
                if _mm:
                    kind, fname, val = _mm.group(1), _mm.group(2), float(_mm.group(3))
                    _field_ranges.setdefault(fname, {})
                    _field_ranges[fname][kind] = val

                # ── Parse surfaceFieldValue output lines ─────────────────
                # e.g. "    areaAverage(inlet) of p = 76.23"
                _sfv = _SURFACE_FIELD_RE.search(line)
                if _sfv:
                    patch_name, fname, val = _sfv.group(1), _sfv.group(2), float(_sfv.group(3))
                    _patch_values.setdefault(patch_name, {})[fname] = val

                # ── Parse volFieldValue output lines ─────────────────
                # e.g. "    volAverage(p) = 101325"
                _vfv = _VOL_FIELD_RE.search(line)
                if _vfv:
                    op, fname, val = _vfv.group(1), _vfv.group(2), float(_vfv.group(3))
                    _volume_integrals[fname] = {"operation": op, "value": val}
            ...
            if event.type in ("run_progress", "run_progress_batch") and mode == SimRunMode.FULL:
                ...
                # fresh set of min/max from the most recent iteration.
                if _field_ranges and built:
                    built[-1]["fieldRanges"] = {
                        fname: {k: v for k, v in vals.items()}
                        for fname, vals in _field_ranges.items()
                    }
                    _field_ranges.clear()

                # Attach accumulated patch-averaged values (surfaceFieldValue)
                if _patch_values and built:
                    built[-1]["patchValues"] = {
                        patch: {k: v for k, v in fields.items()}
                        for patch, fields in _patch_values.items()
                    }
                    _patch_values.clear()

                # Attach accumulated volume integrals (volFieldValue)
                if _volume_integrals and built:
                    built[-1]["volumeIntegrals"] = {
                        fname: {k: v for k, v in data.items()}
                        for fname, data in _volume_integrals.items()
                    }
                    _volume_integrals.clear()

                # ── Persist to local NDJSON file (non-blocking) ──────
                self._progress_writer.append(built)

                # Stream to frontend via WebSocket but skip DB persist for
                # progress events — the data is already saved locally.
                await self.event_bus.emit_sim_event(
                ...
                # ── Convergence assessment (every ~25 steps) ──────────
                self._all_progress.extend(built)
                self._convergence_step_counter += len(built)
                if self._convergence_step_counter >= 25 and len(self._all_progress) >= 10:
                    self._convergence_step_counter = 0
                    try:
                        from simd_agent.convergence import compute_convergence
                        is_transient = (
                        ...
                        assessment = compute_convergence(
                            self._all_progress,
                            is_transient=is_transient,
                        )
                        if assessment:
                            self._last_convergence = assessment
                            await self.event_bus.emit_sim_event(
                                event_type="convergence_update",
        ...
        try:
            ...
            async for event in self.sim_server.stream_events(sim_run_id, on_event=on_sim_event):
                ...
                if self._cancelled.is_set():
                    logger.info("[SIM_SERVER] Run cancelled — killing sim server process")
                    try:
                        await self.sim_server.cancel_run(sim_run_id)
                    except Exception as _ce:
                        logger.warning("[SIM_SERVER] Failed to cancel sim server run: %s", _ce)
                    was_cancelled = True
                    break
            ...
            if test_passed_payload is not None:
                ...
                logger.debug("[SIM] Test passed: %d iterations in %.1fs", iterations_seen, duration)
                return {
                    "success": True,
                    "test_passed": True,
                    "sim_run_id": sim_run_id,
